...
**Generate Agent Skill** を使うと、別の AI agent が VirtualTabs を正しく扱うための構造化された `virtualtabs` skill/rule を生成できます。VirtualTabs のグループが実ファイルシステム上のフォルダーではなく仮想グループであることも明示されます。
