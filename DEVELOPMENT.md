...
VirtualTabs uses three automated test layers:
...
| Product demo recording | `npm run test:ui:demo` | Launches a focused VS Code UI flow for recording product demo footage from a stable fixture workspace. |
...
* `.vscode-test/` is a local cache for downloaded VS Code and installed test extensions. It must stay ignored and should not be committed.
* `out/test/ui/**/*.test.js` is generated by `tsconfig.test.ui.json`. It is test output and is excluded from the VSIX.
...
The demo recording flow is intentionally separate from the regression UI suite:

```bash
npm run test:ui:demo
```

This command compiles the UI demo files, writes deterministic VS Code settings into `test-results/demo-vscode-settings.generated.json`, and runs `out/test/ui/demo/groupOrganize.demo.js` against `test-resources/demo-workspace`. The resulting raw recording output is kept under `test-results/`, which stays ignored and should not be committed.

Notes:

* `test-resources/demo-workspace/` is a committed fixture workspace for product-style demo footage.
* `src/test/ui/demo/` contains the recording helpers, captions, click ripples, and scripted UI flow.
* Demo recording is not part of CI validation; run it manually when preparing release-facing media.
...
└── ui/                   # vscode-extension-tester UI/E2E tests

test-resources/
└── multi-root/           # committed VS Code workspace fixture for UI/E2E tests
```

Before release, run `npx vsce ls --tree` and confirm the VSIX does not include `.vscode-test/`, `out/`, `test-resources/`, `tsconfig.test*.json`, or `jest.config.js`.
